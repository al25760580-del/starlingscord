...
public class IntentModule extends NativeIntentAndroidSpec {
    ...
    public void canOpenURL(String url, @NotNull Promise promise) {
        Intrinsics.checkNotNullParameter(promise, "promise");
        if (url == null || url.length() == 0) {
            promise.reject(new JSApplicationIllegalArgumentException(e.l("Invalid URL: ", url)));
            return;
        }
        try {
            ...
            promise.reject(new JSApplicationIllegalArgumentException(e.m("Could not check if URL '", url, "' can be opened: ", e10.getMessage())));
        }
    }
    ...
    public void getInitialURL(@NotNull Promise promise) {
        ...
        try {
            ...
            promise.reject(new JSApplicationIllegalArgumentException(e.l("Could not get the initial URL : ", e10.getMessage())));
        }
    }
    ...
    public void openSettings(@NotNull Promise promise) {
        ...
        try {
            ...
            promise.reject(new JSApplicationIllegalArgumentException(e.l("Could not open the Settings: ", e10.getMessage())));
        }
    }

    @Override // com.facebook.fbreact.specs.NativeIntentAndroidSpec
    public void openURL(String url, @NotNull Promise promise) {
        Intrinsics.checkNotNullParameter(promise, "promise");
        if (url == null || url.length() == 0) {
            promise.reject(new JSApplicationIllegalArgumentException(e.l("Invalid URL: ", url)));
            return;
        ...
        try {
            ...
            promise.reject(new JSApplicationIllegalArgumentException(e.m("Could not open URL '", url, "': ", e10.getMessage())));
        }
    }

    @Override // com.facebook.fbreact.specs.NativeIntentAndroidSpec
    public void sendIntent(String action, ReadableArray extras, @NotNull Promise promise) {
        Intrinsics.checkNotNullParameter(promise, "promise");
        if (action == null || action.length() == 0) {
            promise.reject(new JSApplicationIllegalArgumentException(g.e("Invalid Action: ", action, ".")));
            return;
        }
        Intent intent = new Intent(action);
        PackageManager packageManager = getReactApplicationContext().getPackageManager();
        if (packageManager == null || intent.resolveActivity(packageManager) == null) {
            promise.reject(new JSApplicationIllegalArgumentException(g.e("Could not launch Intent with action ", action, ".")));
            return;
        }
        if (extras != null) {
            try {
                ...
                for (int i7 = 0; i7 < size; i7++) {
                    ReadableMap map = extras.getMap(i7);
                    if (map != null) {
                        String string = map.getString("key");
                        int i10 = WhenMappings.$EnumSwitchMapping$0[map.getType(EXTRA_MAP_KEY_FOR_VALUE).ordinal()];
                        if (i10 == 1) {
