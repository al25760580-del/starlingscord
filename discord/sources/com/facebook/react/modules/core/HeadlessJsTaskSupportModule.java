...
public class HeadlessJsTaskSupportModule extends NativeHeadlessJsTaskSupportSpec {
    ...
    public void notifyTaskFinished(double taskIdDouble) {
        ...
        if (companion2.isTaskRunning(i7)) {
            companion2.finishTask(i7);
        } else {
            o8.a.t(HeadlessJsTaskSupportModule.class, "Tried to finish non-active task with id %d. Did it time out?", Integer.valueOf(i7));
        }
    }
    ...
    public void notifyTaskRetry(double taskIdDouble, @NotNull Promise promise) {
        ...
        HeadlessJsTaskContext companion2 = companion.getInstance(reactApplicationContext);
        if (companion2.isTaskRunning(i7)) {
            promise.resolve(Boolean.valueOf(companion2.retryTask(i7)));
        } else {
            o8.a.t(HeadlessJsTaskSupportModule.class, "Tried to retry non-active task with id %d. Did it time out?", Integer.valueOf(i7));
