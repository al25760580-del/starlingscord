...
public interface LayoutMetricsConversions {
    ...
    public static final class Companion {
        ...
        public final YogaMeasureMode getYogaMeasureMode(float minSize, float maxSize) {
            if (minSize == maxSize) {
                return YogaMeasureMode.f5461e;
            }
            return Float.isInfinite(maxSize) ? YogaMeasureMode.f5460d : YogaMeasureMode.f5462i;
        }
    ...
    static float getMaxSize(int i7) {
        return INSTANCE.getMaxSize(i7);
    }

    static float getMinSize(int i7) {
        return INSTANCE.getMinSize(i7);
    }

    @NotNull
    static YogaMeasureMode getYogaMeasureMode(float f2, float f7) {
        return INSTANCE.getYogaMeasureMode(f2, f7);
    }

    static float getYogaSize(float f2, float f7) {
        return INSTANCE.getYogaSize(f2, f7);
    }
}
