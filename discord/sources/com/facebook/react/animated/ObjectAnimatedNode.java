...
public final class ObjectAnimatedNode extends AnimatedNode {
    ...
    private final JavaOnlyArray collectViewUpdatesHelper(ReadableArray source) {
        ...
        for (int i7 = 0; i7 < size; i7++) {
            switch (WhenMappings.$EnumSwitchMapping$0[source.getType(i7).ordinal()]) {
                case 1:
                    javaOnlyArray.pushNull();
                    break;
                case 2:
                    javaOnlyArray.pushBoolean(source.getBoolean(i7));
                    break;
                case 3:
                    javaOnlyArray.pushDouble(source.getDouble(i7));
                    break;
                case 4:
                    javaOnlyArray.pushString(source.getString(i7));
                    break;
                case 5:
                    ReadableMap map = source.getMap(i7);
                    if (map != null && map.hasKey(NODE_TAG_KEY) && map.getType(NODE_TAG_KEY) == ReadableType.Number) {
                        ...
                        javaOnlyArray.pushMap(collectViewUpdatesHelper(source.getMap(i7)));
                    }
                    break;
                case 6:
                    javaOnlyArray.pushArray(collectViewUpdatesHelper(source.getArray(i7)));
                    break;
                default:
                    throw new n();
            }
