...
public final class JSTouchDispatcher {
    ...
    private final void dispatchCancelEvent(MotionEvent androidEvent, EventDispatcher eventDispatcher) {
        if (this.targetTag == -1) {
            o8.a.v(ReactConstants.TAG, "Can't cancel already finished gesture. Is a child View trying to start a gesture from an UP/CANCEL event?");
            return;
        }
        io.sentry.config.a.e(!this.childIsHandlingNativeGesture, "Expected to not have already sent a cancel for this gesture");
        ...
        int i7 = this.targetTag;
        TouchEventType touchEventType = TouchEventType.CANCEL;
        long j = this.gestureStartTime;
        float[] fArr = this.targetCoordinates;
        eventDispatcher.dispatchEvent(companion.obtain(surfaceId, i7, touchEventType, androidEvent, j, fArr[0], fArr[1], this.touchEventCoalescingKeyHelper));
    }
    ...
    public final void handleTouchEvent(@NotNull MotionEvent ev, @NotNull EventDispatcher eventDispatcher, ReactContext reactContext) {
        Intrinsics.checkNotNullParameter(ev, "ev");
        Intrinsics.checkNotNullParameter(eventDispatcher, "eventDispatcher");
        int action = ev.getAction() & 255;
        if (action == 0) {
            if (this.targetTag != -1) {
                o8.a.g(ReactConstants.TAG, "Got DOWN touch before receiving UP or CANCEL from last gesture");
            }
            this.childIsHandlingNativeGesture = false;
            ...
            long j = this.gestureStartTime;
            float[] fArr = this.targetCoordinates;
            eventDispatcher.dispatchEvent(companion.obtain(surfaceId, i7, touchEventType, ev, j, fArr[0], fArr[1], this.touchEventCoalescingKeyHelper));
            return;
        }
        ...
        if (i10 == -1) {
            o8.a.g(ReactConstants.TAG, "Unexpected state: received touch event but didn't get starting ACTION_DOWN for this gesture before");
            return;
        }
        if (action == 1) {
            ...
            eventDispatcher.dispatchEvent(companion2.obtain(surfaceId2, i11, touchEventType2, ev, j5, fArr2[0], fArr2[1], this.touchEventCoalescingKeyHelper));
        ...
        if (action == 2) {
            ...
            long j7 = this.gestureStartTime;
            float[] fArr3 = this.targetCoordinates;
            eventDispatcher.dispatchEvent(companion3.obtain(surfaceId3, i12, touchEventType3, ev, j7, fArr3[0], fArr3[1], this.touchEventCoalescingKeyHelper));
            return;
        }
        if (action == 5) {
            ...
            eventDispatcher.dispatchEvent(companion4.obtain(surfaceId4, i13, touchEventType4, ev, j10, fArr4[0], fArr4[1], this.touchEventCoalescingKeyHelper));
            return;
        }
        if (action == 6) {
            ...
            eventDispatcher.dispatchEvent(companion5.obtain(surfaceId5, i14, touchEventType5, ev, j11, fArr5[0], fArr5[1], this.touchEventCoalescingKeyHelper));
            return;
        }
        if (action != 3) {
            o8.a.v(ReactConstants.TAG, "Warning : touch event was ignored. Action=" + action + " Target=" + i10);
            return;
        }
        if (this.touchEventCoalescingKeyHelper.hasCoalescingKey(ev.getDownTime())) {
            dispatchCancelEvent(ev, eventDispatcher);
        } else {
            o8.a.g(ReactConstants.TAG, "Received an ACTION_CANCEL touch event for which we have no corresponding ACTION_DOWN");
        }
    ...
    public final void onChildStartedNativeGesture(@NotNull MotionEvent androidEvent, @NotNull EventDispatcher eventDispatcher, ReactContext reactContext) {
        Intrinsics.checkNotNullParameter(androidEvent, "androidEvent");
        Intrinsics.checkNotNullParameter(eventDispatcher, "eventDispatcher");
        if (this.childIsHandlingNativeGesture) {
            return;
