...
public final class HeadlessJsTaskContext {
    ...
    public static final void finishTask$lambda$4(HeadlessJsTaskContext headlessJsTaskContext, int i7) {
        Iterator<HeadlessJsTaskEventListener> it = headlessJsTaskContext.headlessJsTaskEventListeners.iterator();
        while (it.hasNext()) {
            it.next().onHeadlessJsTaskFinish(i7);
        }
    }
    ...
    private final void scheduleTaskTimeout(int taskId, long timeout) {
        b bVar = new b(this, taskId, 0);
        this.taskTimeouts.append(taskId, bVar);
        UiThreadUtil.runOnUiThread(bVar, timeout);
    }
    ...
    public final synchronized void finishTask(int taskId) {
        ...
        if (zRemove) {
            UiThreadUtil.runOnUiThread(new b(this, taskId, 1));
        }
    }
    ...
    public final synchronized boolean retryTask(int taskId) {
        ...
        if (retryPolicy != null && retryPolicy.canRetry()) {
            removeTimeout(taskId);
            UiThreadUtil.runOnUiThread(new a(this, new HeadlessJsTaskConfig(headlessJsTaskConfig.getTaskKey(), headlessJsTaskConfig.getData(), headlessJsTaskConfig.getTimeout(), headlessJsTaskConfig.getIsAllowedInForeground(), retryPolicy.update()), taskId, 0), retryPolicy.getDelay());
            return true;
        }
        return false;
    }
    ...
    public final synchronized void startTask(HeadlessJsTaskConfig taskConfig, int taskId) {
        try {
            ...
            io.sentry.config.a.g(reactContext, "Tried to start a task on a react context that has already been destroyed");
            ReactContext reactContext2 = reactContext;
            if (reactContext2.getLifecycleState() == LifecycleState.RESUMED && !taskConfig.getIsAllowedInForeground()) {
            ...
            if (reactContext2.hasActiveReactInstance()) {
                ((AppRegistry) reactContext2.getJSModule(AppRegistry.class)).startHeadlessTask(taskId, taskConfig.getTaskKey(), taskConfig.getData());
            } else {
