...
public final class OnScrollDispatchHelper {
    ...
    public final boolean onScrollChanged(int x5, int y5) {
        long jUptimeMillis = SystemClock.uptimeMillis();
        long j = this.lastScrollEventTimeMs;
        boolean z5 = (jUptimeMillis - j <= 10 && this.prevX == x5 && this.prevY == y5) ? false : true;
        if (jUptimeMillis - j != 0) {
            this.xFlingVelocity = (x5 - this.prevX) / (jUptimeMillis - j);
            this.yFlingVelocity = (y5 - this.prevY) / (jUptimeMillis - j);
        }
